...
This project-agnostic template uses the same operating guidance as the template usage guide.
...
## 6. Create a task
```bash
cp .agent/tasks/TEMPLATE.yaml .agent/current-task.yaml
```
Edit objective, acceptance criteria, allowed paths, forbidden paths, autonomy and approval gates.
...
You normally need only this single primary chat. Specialist chats are internal delegation details.
